...
fn computeMain(@builtin(global_invocation_id) pixel: vec3u) {
    ...
    let c0: f32 = colorAt(tex_7, loc.x, loc.y)[channel];
    let c1: f32 = colorAt(tex_8, loc.x, loc.y)[channel];
    let c2: f32 = colorAt(tex_9, loc.x, loc.y)[channel];
    let c3: f32 = c2; // unclear why

    textureStore(tex_out, vec2u(pixel.x, pixel.y), vec4f(c0, c1, c2, c3));
}
